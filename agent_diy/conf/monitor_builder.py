...
def build_monitor():
    ...
    m.end_panel()
    m.add_panel(name="advantage标准差", name_en="adv_std", type="line")
    m.add_metric(metrics_name="adv_std", expr="avg(adv_std{})")
    m.end_panel()
    m.add_panel(name="adv范围", name_en="adv_range", type="line")
    m.add_metric(metrics_name="adv_min", expr="avg(adv_min{})")
    m.add_metric(metrics_name="adv_max", expr="avg(adv_max{})")
    m.end_panel()
    m.add_panel(name="td_return范围", name_en="td_return_range", type="line")
    m.add_metric(metrics_name="td_return_mean", expr="avg(td_return_mean{})")
    m.add_metric(metrics_name="td_return_min", expr="avg(td_return_min{})")
    m.add_metric(metrics_name="td_return_max", expr="avg(td_return_max{})")
    m.end_panel()
    m.add_panel(name="old_prob范围", name_en="old_prob_range", type="line")
    m.add_metric(metrics_name="old_prob_min", expr="avg(old_prob_min{})")
    m.add_metric(metrics_name="old_prob_max", expr="avg(old_prob_max{})")
    m.end_panel()
    m.add_panel(name="new_prob范围", name_en="new_prob_range", type="line")
    m.add_metric(metrics_name="new_prob_min", expr="avg(new_prob_min{})")
    m.add_metric(metrics_name="new_prob_max", expr="avg(new_prob_max{})")
    m.end_panel()
    m.add_panel(name="ExplainedVar", name_en="explained_var", type="line")
    m.add_metric(metrics_name="explained_var", expr="avg(explained_var{})")
    m.end_panel()
    m.add_panel(name="梯度范数_裁剪前", name_en="grad_norm", type="line")
    m.add_metric(metrics_name="grad_norm", expr="avg(grad_norm{})")
    m.end_panel()
    m.add_panel(name="梯度范数_裁剪后", name_en="grad_norm_post_clip", type="line")
    m.add_metric(metrics_name="grad_norm_post_clip", expr="avg(grad_norm_post_clip{})")
    m.end_panel()
    m.add_panel(name="梯度裁剪比例", name_en="grad_clip_ratio", type="line")
    m.add_metric(metrics_name="grad_clip_ratio", expr="avg(grad_clip_ratio{})")
    m.end_panel()
    m.add_panel(name="实际参数更新RMS", name_en="param_update_rms", type="line")
    m.add_metric(metrics_name="param_update_rms", expr="avg(param_update_rms{})")
    m.end_panel()
    m.add_panel(name="非有限计数", name_en="non_finite_flags", type="line")
    m.add_metric(metrics_name="batch_non_finite_count", expr="avg(batch_non_finite_count{})")
    m.add_metric(metrics_name="loss_non_finite", expr="avg(loss_non_finite{})")
    m.add_metric(metrics_name="grad_non_finite", expr="avg(grad_non_finite{})")
    m.add_metric(metrics_name="param_non_finite", expr="avg(param_non_finite{})")
    m.end_panel()
    m.end_group()
