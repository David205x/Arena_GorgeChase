...
_CURRICULUM: list[tuple[int | None, dict]] = [
    (3000,  {"treasure_count": 9, "buff_count": 2, "monster_interval": 500, "monster_speedup": 700}),
    (6000,  {"treasure_count": 8, "buff_count": 2, "monster_interval": 400, "monster_speedup": 700}),
    (9000,  {"treasure_count": 7, "buff_count": 1, "monster_interval": 400, "monster_speedup": 600}),
    (None, {"treasure_count": 6, "buff_count": 2, "monster_interval": 500, "monster_speedup": 700}),
]
...
# ── timing constants ──────────────────────────────────────────────────────────
_MONITOR_INTERVAL = 30   # seconds: min gap between monitor uploads (train mode)
_METRICS_INTERVAL = 60   # seconds: polling cadence for get_training_metrics()

# ── eval snapshot export ──────────────────────────────────────────────────────
# _EVAL_SNAPSHOT_DIR = Path(__file__).resolve().parent / 'trajectory'
_EVAL_SNAPSHOT_DIR = Path('/workspace/code/trajectory').resolve()
...
class EpisodeRunner:
    ...
    def run_episode(self):
        ...
        while not done:
            # predict
            act_data = self.agent.predict(list_obs_data=[obs_data])
            ...
            if not is_val:
                frame = SampleData(
                    ...
                    act=np.array([act_data.action], dtype=np.float32),
    ...
    def _save_eval_extractor_snapshot(self, step: int) -> None: 
        ...
        if extractor is None or rand > 0.3:
            return

        pid = os.getpid()
        _EVAL_SNAPSHOT_DIR.mkdir(parents=True, exist_ok=True)
        np.save(_EVAL_SNAPSHOT_DIR / f"{self.train_ep_cnt:07d}_{pid}_map_full.npy", extractor.map_full)
        np.save(_EVAL_SNAPSHOT_DIR / f"{self.train_ep_cnt:07d}_{pid}_visit_coverage.npy", extractor.visit_coverage)
